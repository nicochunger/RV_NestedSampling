...
methods.remove('')
order = [-3, 1, 0, 3, 4, 2, -2, 5, 6, 7, 8, 9, 10, 11, -1]
...
x = np.arange(1, 7)
offset = np.linspace(-0.45, 0.45, 15)
    ...
    for m, method in enumerate(methods):
        if method == 'PolyChord':
            ax[i].plot(x+offset[m], broad_Zs[i][0].loc[method] -
                       broad_Zs[i][1], 'r.', label=method)
        else:
            ax[i].plot(x+offset[m], broad_Zs[i][0].loc[method] -
                       broad_Zs[i][1], 'b.', label=method)
