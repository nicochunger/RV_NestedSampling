    ...
    for i in range(4):
        for m, method in enumerate(methods):
            if method == 'PolyChord-nlive25nDims-iter50':
                ax[i].plot(x+offset[m], Zs_med[i][0].loc[method] -
                           Zs_med[i][1], 'r.', ms=8, label=method)
            elif method == 'PolyChord-nlive2000-iter6':
                ax[i].plot(x+offset[m], Zs_med[i][0].loc[method] -
                           Zs_med[i][1], 'g.', ms=8, label=method)
            elif method == 'multirun-multinest-nlive2000-eff0.3':
                ax[i].plot(x+offset[m], Zs_med[i][0].loc[method] -
                           Zs_med[i][1], 'k.', ms=8, label=method)
            else:
                ax[i].plot(x+offset[m], Zs_med[i][0].loc[method] -
                           Zs_med[i][1], '.', color='xkcd:sky blue', label='_nolegend_')
